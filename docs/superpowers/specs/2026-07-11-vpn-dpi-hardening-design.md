...
Дополнительное требование пользователя: категория инбаундов с прямым выходом в интернет с публичного IP RuVDS (<RUVDS_IP>) — не туннелировать в Hetzner. Нужно для сервисов, доступных только с российских IP.
...
4. Подключиться существующим ключом → `curl ipinfo.io/json` → должен показать **Hetzner IP** (<HETZNER_IP>) — старые инбаунды работают через wg.
5. Подключиться новым direct-exit ключом → `curl ipinfo.io/json` → **RuVDS IP** (<RUVDS_IP>).
