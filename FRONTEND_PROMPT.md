...
- Две кнопки:
  - **"Запустить"** → `POST /api/telemt/setup` → `{ "message": "telemt настроен и запущен" }`. **Важно:** при первом запуске бэкенд скачивает бинарник (~5-10 сек), создаёт системного пользователя, генерирует конфиг и запускает сервис. Показывать loading/spinner.
