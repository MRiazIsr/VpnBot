...
func GenerateRuVDSConfig() ([]byte, error) {
	...
	cfg := SingBoxConfig{
		...
		Route: &RouteConfig{
			...
			// Не "wg-out". Туннель wg0 RuVDS↔Hetzner не передал ни одного
			// пакета за всё время существования: 02.08.2026 счётчик правила
			// ufw на udp/51820 показывал packets 0, `wg show` — ноль
			// рукопожатий. Пока DNAT перехватывает DE-порты в PREROUTING
			// раньше локального сокета, это незаметно; но стоит DNAT слететь,
			// и весь трафик по умолчанию уходит в мёртвый outbound — то есть
			// в тишину, которую нечем диагностировать.
			//
			// "direct" в том же сценарии даёт рабочий выход с российским
			// адресом. Это не тот экзит, который задумывался для DE-*, зато
			// это работающий экзит, а не чёрная дыра.
			Final: "direct",
		},
	}
